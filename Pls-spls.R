...
plotVar(pls3.result , var.names = FALSE)
...
#choix de 2 composantes à inclure
...
spls_prot_gene <- spls(data.scaled_mRNA,data.scaled_prot , ncomp = 2 , mode = 'regression')
...
final<- spls(data.scaled_mRNA,data.scaled_prot, ncomp = optimal.ncomp, 
                         keepX = optimal.keepX,
                         keepY = optimal.keepY,
                         mode = "regression")
